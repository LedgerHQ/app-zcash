...
void btchip_bagl_user_action_signtx(unsigned char confirming, unsigned char direct) {
    ...
    if (confirming) {
        ...
        if (btchip_context_D.usingOverwinter) {
            CX_ASSERT(cx_hash_no_throw(&btchip_context_D.transactionHashFull.blake2b.header, CX_LAST, hash, 0, hash, 32));
        }
        else {
            CX_ASSERT(cx_hash_no_throw(&btchip_context_D.transactionHashFull.sha256.header, CX_LAST,
                hash, 0, hash, 32));
            PRINTF("Hash1\n%.*H\n", sizeof(hash), hash);
